...
struct AgentlyRunner: AsyncParsableCommand {
    ...
    private func executeTask(_ taskDescription: String) async throws {
        ...
        guard let actions = plannerResult["actions"] as? [String] else {
        ...
        for (index, actionString) in actions.enumerated() {
            logger.info("Executing action \(index + 1)/\(actions.count): \(actionString)")
            
            do {
                let action = parseComputer13Action(actionString, in: currentGraph)
                ...
                if !result.success {
                    ...
                    if let recoveryActions = recoveryPlan["actions"] as? [String], !recoveryActions.isEmpty {
                        logger.info("Attempting recovery with \(recoveryActions.count) actions")
                        
                        // Execute recovery actions
                        var recoverySuccessful = true
                        for (recoveryIndex, recoveryActionString) in recoveryActions.enumerated() {
                            logger.info("Executing recovery action \(recoveryIndex + 1)/\(recoveryActions.count): \(recoveryActionString)")
                            
                            do {
                                let recoveryAction = parseComputer13Action(recoveryActionString, in: currentGraph)
                ...
                if result.success {
                    ...
                    try runLogger.saveGraphToRunDirectory(currentGraph, filename: "\(String(format: "%02d", index + 1))_after_\(action.actionType.rawValue)", runDir: runDir)
                }
    ...
    private func callPythonPlannerForRecovery(
        ...
        let failedActionData = try JSONSerialization.data(withJSONObject: ["action": failedAction], options: .prettyPrinted)
        ...
        let completedActionsData = try JSONSerialization.data(withJSONObject: completedActions.map { result in
            [
                "action": result.action.actionString,
                "success": result.success
            ]
    ...
    private func parseComputer13Action(_ actionString: String, in graph: UIGraph) -> SkillAction {
        // Use the Computer13ActionParser to parse the action
        if let action = Computer13ActionParser.parse(actionString) {
            // If the action has a target element ID, try to resolve it
            if let targetElementId = action.targetElementId {
                let resolvedElementId = resolveElementId(targetElementId, in: graph)
                return SkillAction(
                    actionString: action.actionString,
                    actionType: action.actionType,
                    targetElementId: resolvedElementId,
        ...
        return SkillAction(
            ...
            targetElementId: nil,
            parameters: [:],
            description: "Unknown action: \(actionString)"
        )
    }
